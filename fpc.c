...
static MFILE _mstdin;
static MFILE _mstdout;

static MFILE * mstdin  = &_mstdin;
static MFILE * mstdout = &_mstdout;

static void
minit(void const * const inbuf, void * const outbuf, size_t const n)
{
  mstdin->base   = (void*)inbuf;
  mstdin->curr   = (void*)inbuf;
  mstdin->n      = n;
...
static size_t
mread(void *ptr, size_t size, size_t nmemb, MFILE *stream)
{
  size_t nnmemb = size*nmemb > stream->n ? stream->n/size : nmemb;

  if (nnmemb > 0) {
    memcpy(ptr, stream->curr, size*nnmemb);
    stream->curr += (size*nnmemb);
    stream->n -= (size*nnmemb);
  }
...
static size_t
mwrite(void const *ptr, size_t size, size_t nmemb, MFILE *stream)
{
  if (nmemb > 0) {
    memcpy(stream->curr, ptr, size*nmemb);
    stream->curr += (size*nmemb);
    stream->n += (size*nmemb);
  }
...
fpc_compress(
  long predsizem1,
  void const * const inmem,
  void * const outmem,
  size_t const n
)
{
  static long long *fcm = NULL, *dfcm = NULL;
  ...
  minit(inmem, outmem, n);

  outbuf[0] = predsizem1;
  ioc = mwrite(outbuf, 1, 1, mstdout);
  assert(1 == ioc);
  ...
  intot = mread(inbuf, 8, SIZE, mstdin);
  while (0 < intot) {
    val = inbuf[0];
    out = 6 + ((intot + 1) >> 1);
    *((long long *)&outbuf[(out >> 3) << 3]) = 0;
    ...
    outbuf[4] = out >> 8;
    outbuf[5] = out >> 16;
    ioc = mwrite(outbuf, 1, out, mstdout);
    assert(ioc == out);
    intot = mread(inbuf, 8, SIZE, mstdin);
  }
...
size_t
fpc_decompress(void const * const inmem, void * const outmem, size_t const n)
{
  static long long *fcm = NULL, *dfcm = NULL;
  ...
  if (1 != ioc) {
    ...
    do {
      ...
      ioc = mwrite(outbuf, 8, intot, mstdout);
      assert(ioc == intot);
      intot = 0;
      if ((end - 6) >= in) {
        intot = inbuf[in + 2];
